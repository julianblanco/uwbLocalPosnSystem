...
void DW1000RangingClass::generalStart(){
    ...
    // anchor starts in receiving mode, awaiting a ranging poll message
        ...
        Serial1.print("Unique ID: "); Serial1.print(msg);
        char string[6];
        sprintf(string, "%02X:%02X", _currentShortAddress[0], _currentShortAddress[1]);
        Serial1.print(" short: ");Serial1.println(string);
...
void DW1000RangingClass::startAsAnchor(char address[],  const byte mode[]){
    ...
    _currentShortAddress[0]=222;
    _currentShortAddress[1]=222;
...
void DW1000RangingClass::removeNetworkDevices(short index){
    Serial1.println("removeNetworkDevices");
...
DW1000Device* DW1000RangingClass::getDistantDevice(){
 ...
 Serial1.println("getDistantDevice");
...
void DW1000RangingClass::checkForInactiveDevices(){
    for(int i=0; i<_networkDevicesNumber; i++){
        if(_networkDevices[i].isInactive()){
            ...
            Serial1.println(_networkDevices[i].getShortAddress(), HEX);
...
void DW1000RangingClass::loop(){
        ...
        if(_type==ANCHOR){
            if(messageType == POLL_ACK) {
                ...
                DW1000.getTransmitTimestamp(myDistantDevice->timePollAckSent);
                 Serial1.println("Anchor Loop");
            }
        }
        else if(_type==TAG){
             Serial1.println("tag stuff");
            if(messageType == POLL) {
    ...
    if(_receivedAck){
        ...
        if(messageType==BLINK && _type==ANCHOR){ 
            ...
            {
                Serial1.print("blink; 1 device added ! -> "); 
                Serial1.print(" short:");
                Serial1.println(myTag.getShortAddress(), HEX);
            ...
            {
                Serial1.print("ranging init; 1 device added ! -> ");
                Serial1.print(" short:");
                Serial1.println(myAnchor.getShortAddress(), HEX);
            }
