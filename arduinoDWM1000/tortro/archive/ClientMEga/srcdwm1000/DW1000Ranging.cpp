...
void DW1000RangingClass::removeNetworkDevices(short index){
    Serial.println("removeNetworkDevices");
...
DW1000Device* DW1000RangingClass::getDistantDevice(){
 ...
 Serial.println("getDistantDevice");
        ...
        if(_type==ANCHOR){
            if(messageType == POLL_ACK) {
                ...
                DW1000.getTransmitTimestamp(myDistantDevice->timePollAckSent);
                 Serial.println("Anchor Loop");
            }
        }
        else if(_type==TAG){
             Serial.println("tag stuff");
            if(messageType == POLL) {
